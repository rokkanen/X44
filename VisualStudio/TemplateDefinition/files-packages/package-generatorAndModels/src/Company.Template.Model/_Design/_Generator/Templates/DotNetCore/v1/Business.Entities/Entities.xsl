...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="RelationShip">
    <xsl:variable name="type">
      <xsl:choose>
        <xsl:when test="@relationType='one-to-many'"><xsl:value-of select="$listInterface"/>&lt;<xsl:value-of select="@type"/>&gt;</xsl:when>
        <xsl:when test="@relationType='many-to-many'"><xsl:value-of select="$listInterface"/>&lt;<xsl:value-of select="@relationName"/>&gt;</xsl:when>
        <xsl:otherwise><xsl:value-of select="@type"/></xsl:otherwise>
      </xsl:choose>
    </xsl:variable>
    <xsl:variable name="name">
      <xsl:choose>
        <xsl:when test="@relationType='many-to-many'"><xsl:value-of select="@relationName"/>Rels</xsl:when>
  ...
  <xsl:template name="relationManyToMany">
    <xsl:for-each select="RelationShips/RelationShip[@relationType='many-to-many']">
        public virtual void Add<xsl:value-of select="@relationName"/>(<xsl:value-of select="@relationName"/>&#160;item)
        {
            this.<xsl:value-of select="@relationName"/>Rels.Add(item);
        }
    </xsl:for-each>
  </xsl:template>
