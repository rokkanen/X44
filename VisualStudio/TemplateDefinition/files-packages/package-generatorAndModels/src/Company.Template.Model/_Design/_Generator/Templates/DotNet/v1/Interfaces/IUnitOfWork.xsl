...
namespace <xsl:value-of select="$namespace"/>.Interfaces
{
    ...
    public interface I<xsl:value-of select="$dbNameContext"/>UnitOfWork : IUnitOfWork, IUnitOfWorkAsync
  ...
  <xsl:template match="Entity">
    ...
    <xsl:if test="$selected=1">
        I<xsl:value-of select="@name"/>Repository <xsl:value-of select="@name"/>Repository { get; }</xsl:if>
  </xsl:template>
