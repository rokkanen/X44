...
namespace <xsl:value-of select="$namespace"/>.Model
{
  ...
  <xsl:template name="body">
    ...
    public partial interface I<xsl:value-of select="@name"/>Service : IService&lt;<xsl:value-of select="$dto"/>,<xsl:value-of select="$typeId"/>&gt; 
  ...
  <xsl:template match="RelationShip">
    ...
    <xsl:if test="@relationType='many-to-one'">
        IEnumerable&lt;<xsl:value-of select="$dto"/>&gt; GetBy<xsl:value-of select="@type"/>(<xsl:value-of select="$typeIdRelation"/> id);</xsl:if>
  </xsl:template>
