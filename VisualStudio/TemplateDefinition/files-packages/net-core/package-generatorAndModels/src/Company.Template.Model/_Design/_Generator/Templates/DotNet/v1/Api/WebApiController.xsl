...
namespace <xsl:value-of select="$namespace"/>.Api
{
  ...
  <xsl:template name="body">
    [RoutePrefix("<xsl:value-of select="@routePrefix"/>")]
    public partial class <xsl:value-of select="@id"/>Controller : ApiController
   ...
   <xsl:template name="http_get_template">  
    ...
    <xsl:variable name="endCodeInit">
       <xsl:if test="Parameters/Parameter">
        ...
        [SwaggerResponse(HttpStatusCode.OK, Description = "OK, Request succeed.", Type = typeof(<xsl:value-of select="$response"/>))]
        [SwaggerResponse(HttpStatusCode.NotFound, Description = "No data for request.", Type = typeof(string))]
        [SwaggerResponse(HttpStatusCode.Unauthorized, Description = HttpConstantMessages.Unauthorized)]
        [SwaggerResponse(HttpStatusCode.InternalServerError, Description = "Unexpected error.")]
        public async Task&lt;HttpResponseMessage&gt; <xsl:value-of select="@name"/>(<xsl:value-of select="$parameterCall"/>)
        {
            _log.Info("GET: " + Request.RequestUri.AbsoluteUri);
            <xsl:value-of select="@query"/> query = new <xsl:value-of select="@query"/>();            
            <xsl:value-of select="$parameterInitialize"/>
            query.Execute();      
            
            if (query.Ok) {
                _log.InfoFormat("Response 200:OK / {0} rec.", query.Result.Count());
                return Request.CreateResponse(HttpStatusCode.OK, query.Result);
            }
            else {
                _log.Info("Response 404: No Data");
                return Request.CreateResponse(HttpStatusCode.NotFound, "no_data");
            }
        }
  </xsl:template>
   ...
   <xsl:template name="http_post_template">  
    ...
    <xsl:variable name="endCodeInit">
       <xsl:if test="Parameters/Parameter">
        ...
        [SwaggerResponse(HttpStatusCode.OK, Description = "OK, Request succeed.", Type = typeof(<xsl:value-of select="$response"/>))]
        [SwaggerResponse(HttpStatusCode.Unauthorized, Description = HttpConstantMessages.Unauthorized)]
        [SwaggerResponse(HttpStatusCode.NotAcceptable, Description = "System returned an error.")]
        public async Task&lt;HttpResponseMessage&gt; <xsl:value-of select="@name"/>(<xsl:value-of select="$parameterCall"/>)
        {
            _log.Info("POST: " + Request.RequestUri.AbsoluteUri);
            <xsl:value-of select="@query"/> command = new <xsl:value-of select="@query"/>();            
            <xsl:value-of select="$parameterInitialize"/>
            command.Execute();     
            
            if (command.Ok) {
                _log.Info("Response 200:OK");
                return Request.CreateResponse(HttpStatusCode.OK);
            }
            else {
                _log.Info("Response 406: " + command.ErrorMessage);
                return Request.CreateResponse(HttpStatusCode.NotAcceptable, command.ErrorMessage);
            }
        }
  </xsl:template>
