   ...
   <xsl:template name="http_get_template">  
    ...
    <xsl:variable name="endCodeInit">
       <xsl:if test="Parameters/Parameter">
        ...
        /// <response code="200">OK, Request succeed.</response>
        /// <response code="400">Error in query.</response>
        /// <response code="401">Unauthorized, the correct api_key must be specified.</response>
        /// <response code="404">No data for request.</response>
        /// <response code="500">Unexpected error.</response>
        [<xsl:value-of select="@httpMethod"/>]
        [Route("<xsl:value-of select="@route"/>")]        
        [ProducesResponseType(typeof(<xsl:value-of select="$response"/>), (int)HttpStatusCode.OK)]
        ...
        public async Task&lt;IActionResult&gt; <xsl:value-of select="@name"/>(<xsl:value-of select="$parameterCall"/>)
        {
            ...
            <xsl:value-of select="@query"/> query = new <xsl:value-of select="@query"/>(_config);            
            ...
            if (query.Ok) {
                ...
                _log.Info("Response 400: " + query.ErrorMessage);
   ...
   <xsl:template name="http_post_template">  
    ...
    <xsl:variable name="endCodeInit">
       <xsl:if test="Parameters/Parameter">
        ...
        /// <response code="400">Error in query.</response>
        /// <response code="401">Unauthorized, the correct api_key must be specified.</response>
        [ProducesResponseType(Type = typeof(<xsl:value-of select="$response"/>), (int)HttpStatusCode.OK)]
        [ProducesResponseType((int)HttpStatusCode.Unauthorized]
        [ProducesResponseType((int)HttpStatusCode.BadRequest]
        public async Task&lt;IActionResult&gt; <xsl:value-of select="@name"/>(<xsl:value-of select="$parameterCall"/>)
        {
            //_log.Info("POST: " + Request.RequestUri.AbsoluteUri);
            <xsl:value-of select="@query"/> command = new <xsl:value-of select="@query"/>(_config);            
            <xsl:value-of select="$parameterInitialize"/>
            command.Execute();
            if (command.Ok) {
                ...
                _log.Info("Response 400: " + command.ErrorMessage);
