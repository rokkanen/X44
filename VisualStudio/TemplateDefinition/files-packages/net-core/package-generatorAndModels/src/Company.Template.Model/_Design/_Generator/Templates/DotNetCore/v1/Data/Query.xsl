...
namespace <xsl:value-of select="$namespace"/>
{
  ...
  <xsl:template name="body">
    ...
    <xsl:variable name="dto"><xsl:value-of select="@resultType"/></xsl:variable>
    ...
    <xsl:choose>
      <xsl:when test="starts-with($code,'Select') or starts-with($code,'select') or starts-with($code,'SELECT')">
  ...
  <xsl:template name="commandStoredprocedure">
    ...
    <xsl:variable name="hasParameters"><xsl:value-of select="//Query[@name=$queryName]/Parameters"/></xsl:variable>
    <xsl:variable name="resultParameter"><xsl:value-of select="//Query[@name=$queryName]/Parameters/Parameter[@name='Result']/@mapping"/></xsl:variable>
    <xsl:variable name="resultParameterType"><xsl:value-of select="//Query[@name=$queryName]/Parameters/Parameter[@name='Result']/@type"/></xsl:variable>
    ...
    public partial class <xsl:value-of select="$queryName"/>Query: IQuery&lt;IEnumerable&lt;<xsl:value-of select="$response"/>&gt;&gt;
    {   <xsl:apply-templates select="//Query[@name=$queryName]/Parameters" mode="classParameter"/>
        <xsl:if test="//Query[@name=$queryName]/Parameters">      public QueryParameter Parameter {get; private set;}</xsl:if>
        public bool Ok { get {return _ok;}}
        public string ErrorMessage { get {return _errorMessage;}}                
        public IEnumerable&lt;<xsl:value-of select="$response"/>&gt; Result {get; private set;}
        private bool _ok = true;
        private string _errorMessage = "";
        private readonly ILog _log = LogManager.GetLogger(typeof(<xsl:value-of select="$queryName"/>Query));
        private string _connectionString;
                
        public <xsl:value-of select="$queryName"/>Query(IConfiguration config)
        {
          _connectionString = config.GetConnectionString("<xsl:value-of select="$connexionString"/>");
            ...
            try
            {
                using (PerformanceTrace perf = new PerformanceTrace("<xsl:value-of select="$queryName"/>", _log))
                {
                    ...
                    {     
                        ...
                        this.Result = conn.Query&lt;<xsl:value-of select="$response"/>&gt;(sql, param, commandType: CommandType.StoredProcedure);                       
                    }
                }
            }
            catch (Exception ex)
     ...
     <xsl:variable name="privateSet">
      <xsl:choose>
        ...
        <xsl:otherwise></xsl:otherwise>
            ...
            public <xsl:value-of select="@type"/>&#160;<xsl:value-of select="@name"/>&#160;{get; set;}<xsl:value-of select="$assignProperty"/>
</xsl:template>
